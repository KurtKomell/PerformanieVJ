...
void main()
{
    ...
    vec3 wN = wRaw / (wRaw.x + wRaw.y + wRaw.z);
    ...
    if (ubuf.rotation.w < 0.5) {
        vec3 keyRgb = pvjHsl2rgb(vec3(fract(pKey), 1.0, 0.5));
        float metric = 0.0;
        if (mi <= 1) {
            vec3 hsl = pvjRgb2hsl(c);
            float dh = abs(hsl.x - fract(pKey));
            dh = min(dh, 1.0 - dh) * 2.0;
            metric = dh * mix(0.15, 1.0, hsl.y);
        } else if (mi <= 3) {
            metric = length((c - keyRgb) * wRaw);
        } else {
        ...
        if (edgeLo >= edgeHi) edgeHi = min(1.0, edgeLo + pSoftEff * 2.0);
        ...
        float v = 0.0;
        if (mi == 0) v = dot(c, wN);
        else if (mi == 1) v = 1.0 - dot(c, wN);
        else if (mi == 2) v = dot(c, PVJ_LUMA);
        else if (mi == 3) v = 1.0 - dot(c, PVJ_LUMA);
        else if (mi == 4) v = c.r;
        else if (mi == 5) v = c.g;
        else if (mi == 6) v = c.b;
        else if (mi == 7) v = max(max(c.r, c.g), c.b);
        else if (mi == 8) v = min(min(c.r, c.g), c.b);
        else v = 1.0 - max(max(c.r, c.g), c.b);
        float d = abs(v - pKey);
        float pSoftEff = max(pSoft, 0.04);
        float edgeLo = clamp(pTh - pSoftEff, 0.0, 1.0);
        float edgeHi = clamp(pTh + pSoftEff, 0.0, 1.0);
