...
void main()
{
    ...
    float mixAmt = ubuf.params.z;
    int id = pvjEffectId();

    float r = length(uv);
    float a = atan(uv.y, uv.x) + phase;
    float sector = PVJ_PI * 2.0 / seg;

    if (id == 1) { // mirror_quad — 4-way
        a = abs(mod(a + sector * 0.5, sector) - sector * 0.5);
    } else if (id == 2) { // mirror_stripes — horizontal stripes
        uv.y = abs(uv.y);
        fragColor = texture(u_tex, clamp(uv + 0.5, 0.001, 0.999));
        return;
    } else if (id == 3) { // multi_mirror
        seg *= 2.0;
    }
    ...
    vec4 src = texture(u_tex, clamp(uv2, 0.001, 0.999));
    vec4 orig = texture(u_tex, clamp(v_uv, 0.001, 0.999));
    fragColor = mix(orig, src, mixAmt);
}
