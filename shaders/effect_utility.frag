...
void main()
{
    ...
    if (id == 0) { // add_subtract
        float m = ubuf.params.x;
        rgb = mix(rgb, rgb + vec3(m), ubuf.params.y);
        if (int(ubuf.params.z + 0.5) == 1) rgb = max(rgb - vec3(m), 0.0);
    } else if (id == 1 || id == 2 || id == 6) { // mix / fade / opacity
        rgb *= ubuf.params.x;
    } else if (id == 3) { // rgb_shift
        float dist = ubuf.params.x * 0.01;
        float ang = ubuf.params.y;
        vec2 off = vec2(cos(ang), sin(ang)) * dist;
        rgb.r = texture(u_tex, clamp(uv + off, 0.001, 0.999)).r;
        rgb.b = texture(u_tex, clamp(uv - off, 0.001, 0.999)).b;
    } else if (id == 4) { // shift
        uv += vec2(ubuf.params.x, ubuf.params.y) * 0.1;
        rgb = pvjSampleRgb(u_tex, uv);
    } else if (id == 5) { // tilt_shift
        vec2 p = pvjUvCentered(uv);
        float blurAmt = ubuf.params.x * abs(p.y) * 0.05;
        rgb = pvjBlur9(u_tex, uv, vec2(1.0, 0.0), blurAmt);
